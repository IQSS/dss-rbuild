...
Say you have created some code in your project directory and want to add it to the repository and change it later on. To do this, you need to first move the files in your project directory to the repository. Then you need to use three commands: *git add*, *git commit*, and *git push.* Note that sending scripts to the local repository is (perhaps counterintuitively) known as *commiting* them, not adding them. 

First, suppose you make changes to your scripts. You need to *add*  the modified scripts to Git (adding scripts doesn't  add them to the repository - rather, it just lets Git know that you will eventually *commit* or send them to the repository). The scripts must already exist in the repository before they can be added. Then navigate to the correct working directory and type 'git add [your script's filepath]' to the directory. You can add multiple files in a row with no problem. 
...
Lastly, you need to *push* the files, which means sending them to GitHub online. Before you do this, if you created the repository locally and have also created a GitHub repo without adding the README file, you'll need to specify to Git the url to send the commits to. To do this, type 'git remote add origin [your url]' into your command line. Then, you can *push* the file by writing 'git push -u origin master,' which will send the file to the GitHub repo. If you created the repository locally and have also created a GitHub repo adding the README file, then you have to resolve commitment consolidation when you push your file to the GitHub repo. This is why we recommend that you do not initialize the GitHub repo with the README file. If you did not create the repository locally, then you can simply push it to the GitHub repo after you clone the GitHub repo to your local repository. So, it is easier to start a repo on GitHub and then clone it than initializing a repository locally. 
