...
As mentioned previously, the purpose of the namespace is make sure your package works in combination with other packages. For example, if your package includes some data labelled 'lm', the namespace will ensure that using that data in conjunction with the R stats package (which has a function called 'lm') will *not* throw an error. How exactly the namespace does this is slightly beyond the scope of this guide, so if you're dying for specifics, you should [read through this page](http://r-pkgs.had.co.nz/namespace.html). Thankfully, as we just discussed, Roxygen2 will automatically generate a namespace for you when you create documentation with "@export" tags, so you don't need to worry about this too much.
